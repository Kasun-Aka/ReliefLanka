...
export function Volunteers() {
  ...
  const save = async (volunteer: Volunteer) => {
    try {
      ...
      toast.error('Could not save this volunteer. Check the backend connection.');
    }
  };
  ...
  const toggleAvailability = async (volunteer: Volunteer) => {
    ...
    try {
      ...
      toast.error('Could not update volunteer availability.');
    }
  };
        ...
        <tr key={volunteer.id} className="transition-colors duration-150 ease-out hover:bg-brand-50/60">
              ...
              <Td align="right">
                <div className="flex items-center justify-end gap-1.5">
                  {user?.role !== 'coordinator' && <Button size="sm" onClick={() => toggleAvailability(volunteer)}>
                    {volunteer.availability === 'Available' ? 'Deploy' : 'Stand down'}
                  </Button>}
                  {user?.role !== 'coordinator' && <Button
                size="sm"
                variant="ghost"
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            toast.error('Could not remove this volunteer.');
          }
        }} />
